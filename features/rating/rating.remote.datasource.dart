...
import 'package:recommend/features/rating/rating.model.dart';
...
class RatingRemoteDataSourceImplementation implements RatingRemoteDataSource {
  ...
  @override
  Future<List<RatingModel>> getEveryRatingByUser() => _getRatingsFromUrl(
      'https://googlapi.herokuapp.com/firebase/database/ratings');

  Future<List<RatingModel>> _getRatingsFromUrl(String url) async {
  ...
  List<RatingModel> _checkResponseWithList(http.Response response) {
    if (response.statusCode == 200) {
      ...
      if (decodedJson == "null") {
        ...
        var ratings = (json.decode(responseBody) as List)
            .map((e) => new RatingModel.fromJson(e))
            .toList();
        return ratings;
      }
    } else if (response.statusCode >= 500) {
